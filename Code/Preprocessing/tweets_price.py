...
for i in df.date:
    ...
    for j in df.date:
        if j==cur1:
            df["t-1"][j]=(df.loc[j, "price"]-df.loc[i, "price"])/df.loc[i, "price"]
        if j==cur2:
            df["t-2"][j]=(df.loc[j, "price"]-df.loc[i, "price"])/df.loc[i, "price"]
        if j==cur3:
            df["t-3"][j]=(df.loc[j, "price"]-df.loc[i, "price"])/df.loc[i, "price"]
        if j==cur4:
            df["t-4"][j]=(df.loc[j, "price"]-df.loc[i, "price"])/df.loc[i, "price"]
        if j==cur5:
            df["t-5"][j]=(df.loc[j, "price"]-df.loc[i, "price"])/df.loc[i, "price"]
        if j==cur6:
            df["t-6"][j]=(df.loc[j, "price"]-df.loc[i, "price"])/df.loc[i, "price"]

dt1 = pd.to_datetime("2019-01-01")

df=df[(df['date'] >= dt1)].reset_index()
df = df.drop(columns=['DateIndex'])
write_pickle(output_pickle_path, "price_preprocessed_sr3.pkl", df)
